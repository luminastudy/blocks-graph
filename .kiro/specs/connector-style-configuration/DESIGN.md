...
function migrateEdgeStyle(config: Partial<RendererConfig>): RendererConfig {
  if (config.edgeStyle) {
    ...
    config.connectorStyle = {
      ...
      stroke: prereq?.stroke || parent?.stroke || DEFAULT_CONNECTOR_STYLE.stroke,
...
--edge-stroke-width: <number>
--edge-dash-pattern: <string>
--edge-arrow-enabled: true | false
--edge-arrow-size: <number>
--edge-curve-type: straight | bezier | arc
--edge-curve-tension: <number>
--edge-animation-enabled: true | false
