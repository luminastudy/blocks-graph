...
- _Gain_: Isolated transformation scope, simpler event coordinate mapping, future-proof for UI overlays
- _Sacrifice_: Slightly more complex DOM structure (additional group element), need to traverse to transform group for updates
...
- _Gain_: Better testability, clearer separation of concerns, easier to extend with features like viewport history
- _Sacrifice_: Additional class increases codebase size, indirection adds minimal call overhead
...
- _Gain_: ~40% less event handling code, unified coordinate system, better multi-input device support
- _Sacrifice_: Requires two event systems (Pointer + Touch), older browsers need polyfill (acceptable given target audience)
...
console.warn(
  '[BlocksGraph:Viewport] Invalid zoom limits: min=${min}, max=${max}. Using defaults.'
)
console.error(
  '[BlocksGraph:Viewport] Failed to calculate transform: ${error.message}'
)
```
